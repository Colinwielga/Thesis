...
Do While ((CTR > a) And (Not Found))
a = 1 + a

If movie = num(a) Or number(a) = movie Or movie = numbers(a) Then
    Found = True
    End If
    Loop
...
Do While ((CTR > d) And (Not Found))
d = 1 + d
If movie = rating(d) Or ratings(d) = movie Then
    Found = True
    End If
    Loop
...
Do While ((Not Found) And (CTR > c))
c = 1 + c
If titles(c) = movie Or tit(c) = movie Or movie = title(c) Then
    Found = True
    End If
    Loop
