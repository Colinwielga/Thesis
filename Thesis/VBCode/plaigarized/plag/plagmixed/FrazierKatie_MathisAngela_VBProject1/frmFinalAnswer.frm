...
'The UCase formatting option for strings is used to avoid user error.
        If OptionYes = True Then
            If UCase(UserAnswer) = Answer(J) Then
                ...
                Select Case J
                Case CTR     'Checks to see if the question is the last in the game.
                ...
                Case Else                      'If it was not the last question but was correct,
    ...
    If Not False = OptionNo Then             'If the user selects the No option button,
        J = -1 + J                       'the Question array goes back to the same question
        frmQuestion.Show
        frmFinalAnswer.Hide             'and the user returns to the Question form.
    End If

    If Not (OptionNo Or OptionYes) Then    'If the user has not selected either but clicks Continue
