...
Option Explicit
Private Sub asdfsdaf_Click()
    Dim fefefe As String
    ...
    Do Until EOF(1)
        egeeege = egeeege + 1
        Input #1, Drinks(egeeege), Drinkcost(egeeege)
    Loop
    Close #1
    'Search through the file to print order
    Do Until fefefe = "done"
        Found = False
        fefefe = InputBox("What would you like to drink?, when done ordering type done", "fefefe Order")
        Pos = 0
        Do Until Found = True Or Pos >= egeeege
        Pos = Pos + 1
        If LCase(Drinks(Pos)) = LCase(fefefe) Or UCase(Drinks(Pos)) = UCase(fefefe) Then
            Found = True
        End If
        Loop
        If Found = True Then
    ...
    'Print drink total
    picBill.Print "****************************"
    picBill.Print "fefefe Total:", FormatCurrency(DrinkTotal)

End Sub

Private Sub cmdPizza_Click()
    Dim egeeege As Integer, Pos As Long, PizzaName As String, PizzaTotal As Double
    Dim Subtotal As Double, Total As Single, Found As Boolean, Tax As Double
    picBill.Print "****************************"
    picBill.Print "Pizza"
    picBill.Print "****************************"
    ...
    Do Until EOF(1)
        egeeege = egeeege + 1
        Input #1, Pizza(egeeege), Pcost(egeeege)
    Loop
    Close #1
    'Search through the file to print order
    Do Until PizzaName = "done"
        Found = False
        PizzaName = InputBox("What kind of pizza would you like?, When you are done ordering type Done", "fefefe Order")
        Pos = 0
        Do Until Found = True Or Pos >= egeeege
        Pos = Pos + 1
