...
Private Sub cmdSearch_Click()
    ...
    Do 'run search until it finds the name of the correct pitcher

        A = A + 1
        If Not X <> Pitchers(A) Then
            Found = True
...
'sort all the Cy Young winners by first name
    ...
    For Pass = 1 To 29 'sort the array alphabetically
        For N = 1 To 30 - Pass
            If Not Pitchers(N) <= Pitchers(N + 1) Then
            Temp = Pitchers(N)
...
Private Sub cmdSort3_Click()
    ...
    While Pass <= 29
        For N = 1 To 30 - Pass
            If Not StrikeOuts(N) >= StrikeOuts(N + 1) Then 'sort by which pitcher had the most strike outs
            Temp = StrikeOuts(N)
                StrikeOuts(N) = StrikeOuts(N + 1)
                StrikeOuts(N + 1) = Temp
                Temp2 = Wins(N)
