...
Private Sub cmdSortActor_Click()
    ...
    For G = 1 To (6 - 1)
        For G2 = 1 To (6 - G)
            If ActorsYF(G2) > ActorsYF(G2 + 1) Then
                TempActor = ActorsYF(G2)
                ActorsYF(G2) = ActorsYF(G2 + 1)
                ActorsYF(G2 + 1) = TempActor
                TempPart = PartYF(G2)
                PartYF(G2) = PartYF(G2 + 1)
                PartYF(G2 + 1) = TempPart
             End If
        Next G2
    Next G
     picResults2.Print "Actor"; Tab(20); "Character"
...
Private Sub cmdSortWinner_Click()
    ...
    For Y = 1 To (9 - 1)
        For Y2 = 1 To (9 - Y)
            If AwardWnnr(Y2) > AwardWnnr(Y2 + 1) Then
                TempWinner = AwardWnnr(Y2)
                AwardWnnr(Y2) = AwardWnnr(Y2 + 1)
                AwardWnnr(Y2 + 1) = TempWinner
                TempAward = AwardYF(Y2)
                AwardYF(Y2) = AwardYF(Y2 + 1)
                AwardYF(Y2 + 1) = TempAward
             End If
        Next Y2
    Next Y
          picResults2.Print "Award"; Tab(60); "Award Winner"
