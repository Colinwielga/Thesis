...
Private Sub lblsignoff_Click()
    ...
    Next i
    Open App.Path & "\gueststatus.txt" For Output As #1
        Print #1, "0"
    Close #1
    End
End Sub
...
Private Sub Timer1_Timer()
    If Timer1 = True Then
        Open App.Path & "\rickstatus.txt" For Input As #1
            Input #1, rickstatus
            Close #1
            If rickstatus = 1 Then
                lblguest.Enabled = True
                lblguest.ForeColor = &H80FF&
                lblguest.Caption = "Richard Motzko"
                buddy = "Richard Motzko"
            Else
