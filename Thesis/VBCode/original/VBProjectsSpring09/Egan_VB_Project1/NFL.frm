...
'frmNFL (National Football League)
'Written by: Sean Egan
'Written on: 3/20/09
'This form allows the user to bet on pro football games.
...
Private Sub cmdDisplay_Click()
    ...
    'Open the file
    Open App.Path & "\NFLGames.txt" For Input As #1
    
    'Read the file into 2 parallel arrays
    Do While Not EOF(1)
        CTR = CTR + 1
        Input #1, AwayTeam(CTR), HomeTeam(CTR)
    Loop
    'Close the file
    Close #1
    ...
    Do While rCTR < 16
        ...
        If Num(r) = 0 Then
            rCTR = rCTR + 1
            Num(r) = r
            lstGames.AddItem AwayTeam(r) & " vs. " & HomeTeam(r) & vbCrLf
        End If
    Loop
