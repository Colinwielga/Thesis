VERSION 5.00
Begin VB.Form frmRegularGrads 
   BackColor       =   &H000080FF&
   Caption         =   "Regular Grads"
   ClientHeight    =   12645
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   18615
   ...
   Begin VB.CommandButton cmdSearchByState 
      BackColor       =   &H8000000E&
      Caption         =   "Search By State"
      Height          =   855
      Left            =   600
      Style           =   1  'Graphical
      TabIndex        =   10
   ...
   Begin VB.Label lblAuthor 
      ...
      Caption         =   "Tim Johnson   3/20    To show some information about Hargraves Regular Grads and let users interact with that info"
      Height          =   615
      Left            =   15360
      TabIndex        =   11
      Top             =   11880
   ...
   End
End
Attribute VB_Name = "frmRegularGrads"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Option Explicit
Dim Last(1 To 150) As String, First(1 To 150) As String, Home(1 To 150) As String, State(1 To 150) As String, College(1 To 150) As String, Award(1 To 150) As String
...
frmRegularGrads.Hide                                'goes back to the Hargrave main form
frmHargrave.Show

End Sub

Private Sub cmdHome_Click()                          'sorts information by the home town the graduate is from
...
Private Sub cmdSearchByLastName_Click()                                             'allows a person to either do a match/stop search or an exhaustive search by last name
Found = False
j = 0
...
Private Sub cmdSearchByState_Click()                                             'allows a person to either do a match/stop search or an exhaustive search by the state the graduate was from
Found = False
j = 0
NumberFound = 0                                                                  'preps the information to be searched

LookingFor = InputBox("Which home state are you looking for?", , "Enter a string")                      'asks the user what they are searching for
...
If SearchOption = 1 Then                                                        'if the user selected a match/stop search
    Do While Not Found And j <= CTR                                             'do until the first match is found
        j = j + 1
        If LookingFor = State(j) Then                                           'if found then keep track of the finding
            Found = True
        End If
    Loop
    If Not Found Then
        picResults.Print LookingFor; " did not graduate from Hargrave in 2005." 'lets the user know the search failed
    Else
        picResults.Print "The first person from this state is listed below"     'preps a table for information to be displayed
        picResults.Print "Last Name"; Tab(15); "First Name"; Tab(35); "Hometown"; Tab(55); "State"; Tab(65); "College"; Tab(105); "Award"
        picResults.Print "***************************************************************************************************************************************************"
...
ElseIf SearchOption = 2 Then                                                    'if the person wants to do an exhaustive search
    For j = 1 To CTR
        If LookingFor = State(j) Then                                           'if the search is a success
            Found = True                                                        'register that something has been found
            NumberFound = NumberFound + 1                                       'add more to the total found
            If NumberFound = 1 Then                                             'only lets the below table be printed once
                picResults.Print "The people from this state are listed below"  'preps a table to display information
                picResults.Print "Last Name"; Tab(15); "First Name"; Tab(35); "Hometown"; Tab(55); "State"; Tab(65); "College"; Tab(105); "Award"
                picResults.Print "***************************************************************************************************************************************************"
            End If                                                  'prints all relevant information
            picResults.Print Last(j); Tab(15); First(j); Tab(35); Home(j); Tab(55); State(j); Tab(65); College(j); Tab(105); Award(j)
    ...
    Next j
    If Not Found Then                                               'lets user know that the search they attempted, failed
        picResults.Print "No one from the great state of "; LookingFor; " graduated from Hargrave in 2005."
    End If                                                          'displays the total number of hits found on the exhaustive search
    picResults.Print "The number of people from the great state of "; LookingFor; ": "; NumberFound
End If
...
j = 0                                                               'preps the information to be displayed
    
    picResults.Cls                                                  'sets up a display table
    picResults.Print "Last Name"; Tab(15); "First Name"; Tab(35); "Hometown"; Tab(55); "State"; Tab(65); "College"; Tab(105); "Award"
    picResults.Print "***************************************************************************************************************************************************"

Do While j <= CTR                                                   'prints all relevant information
    j = j + 1
    picResults.Print Last(j); Tab(15); First(j); Tab(35); Home(j); Tab(55); State(j); Tab(65); College(j); Tab(105); Award(j)
Loop
...
Private Sub Form_Load()                                                                     'loads multiple arrays for this form
    ...
    Open App.Path & "\RegularGrads.txt" For Input As #1                                     'opens the text file to be read
