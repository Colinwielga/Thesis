...
Begin VB.Form frmQ6 
   ...
   Begin VB.CommandButton cmdC 
      Caption         =   "Gelatin Silverprinting"
      Height          =   975
      Left            =   7080
      TabIndex        =   4
      Top             =   8520
   ...
   Begin VB.Label lblQuestion 
      BackColor       =   &H80000010&
      Caption         =   "Recording subjects electrical aura is called "
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   18
         Charset         =   0
...
'message boxes indicating the more likely answer choice and eliminate the possibility of clicking those hint buttons again.
'There is also a countdown timer that gives the user 40 seconds to answer the questions.
...
'adds 1 to computer counter and checks if has been used before. If has then gives an error, if not gives a hint. Hides the button
Computer = Computer + 1

If Computer = 1 Then

    MsgBox "There is a " & FormatPercent(0.68, 0) & " possibility that the answer is " & cmdA.Caption & " or " & cmdC.Caption & ".", , "CPU Output"
...
'subtracts 1 from the FiftyFifty counter; checks if has been used; if yes then gives an error, if not then gives a hint. Hides the button
...
'adds 1 to google counter and checks if has been used before. If has then gives an error, if not gives a hint. Hides the button
