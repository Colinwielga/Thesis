VERSION 5.00
Begin VB.Form frmSJU_Invite_Results 
   BackColor       =   &H000000FF&
   Caption         =   "Form1"
   ClientHeight    =   8550
   ClientLeft      =   60
   ...
   Begin VB.CommandButton cmdFullResults 
      ...
      Width           =   1575
   End
   Begin VB.Label lblCollegville 
      BackColor       =   &H000000FF&
      Caption         =   "Collegeville, MN  9/8/2007"
      BeginProperty Font 
         Name            =   "Rockwell"
         Size            =   15.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmSJU_Invite_Results"
...
Private Sub cmdFullResults_Click()
    ...
    Open App.Path & "\sju_invite_team_results.txt" For Input As #1
    Do Until EOF(1)
        CTRteam = CTRteam + 1
        Input #1, TeamPlace(CTRteam), TeamName(CTRteam), TeamScore(CTRteam)
    Loop
    
    picResults.Print "Place"; Tab(10); "Team"; Tab(25); "Score"
    
    Do While J < CTRteam
        J = J + 1
        picResults.Print TeamPlace(J); Tab(10); TeamName(J); Tab(25); TeamScore(J)
    Loop
    ...
    Open App.Path & "\sju_invite_individual_results.txt" For Input As #2
    Do Until EOF(2)
        CTR = CTR + 1
