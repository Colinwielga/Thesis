...
'Purpose: to determine if any clubs are in debt on a specific date in October and if so how much are they in debt.
...
Do While Number > 31 Or Number < 1 'if number is not between 1 and 31(because of 31 days in October) give error message
    ...
    Number = InputBox("Enter October Date", "Date") 'ask for october date again
Loop 'continue until user enters number between 1 and 31
Open "M:\CS130\Projects\VB 10-21-02\Commtxt.txt" For Input As #1 'open file
...
If Balance(Number) < 0 Then 'check if club is in debt on date specified above
        ...
        picResults.Print Tab(10); "Their Balance is"; FormatCurrency(Balance(Number)) 'if in debt tell user and give user balance also
    Else
        picResults.Print "The Communication Club is not in Debt." 'if not in debt just print that
End If
Close #1 'close file
Open "M:\CS130\Projects\VB 10-21-02\Demstxt.txt" For Input As #1 'open next club's file
For J = 1 To 31 'fill array
    Input #1, Balance(J)
Next J
If Balance(Number) < 0 Then 'check if club is in debt on date specified above
...
End If
Close #1 'close file
Open "M:\CS130\Projects\VB 10-21-02\Fusiontxt.txt" For Input As #1 'open next club's file
For J = 1 To 31 'fill array
    Input #1, Balance(J)
Next J
If Balance(Number) < 0 Then 'check if club is in debt on date specified above
...
End If
Close #1 'close file
Open "M:\CS130\Projects\VB 10-21-02\Rugbytxt.txt" For Input As #1
For J = 1 To 31 'fill array
    Input #1, Balance(J)
Next J
If Balance(Number) < 0 Then 'check if club is in debt on date specified above
...
End If
Close #1 'close file
Open "M:\CS130\Projects\VB 10-21-02\Spanishtxt.txt" For Input As #1
For J = 1 To 31 'fill array
    Input #1, Balance(J)
Next J
If Balance(Number) < 0 Then 'check if club is in debt on date specified above
