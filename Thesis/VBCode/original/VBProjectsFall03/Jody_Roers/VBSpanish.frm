...
Open "M:\CS130\Projects\VB 10-21-02\Spanishtxt.txt" For Input As #1 'open file
...
If Balance(D) > M Then 'checking if club has enough money to subtract that withdrawal
        ...
        picResults.Print "The Spanish Club has enough money to cover this. The new balance is "; FormatCurrency(Sum) 'printing new balance
    Else
        picResults.Print "The Spanish Club does not have enough money to cover this expense." 'letting user know that comm club doesn't have enough money
End If
Close #1 'close file
End Sub
